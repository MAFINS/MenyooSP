...
namespace sub
{
	namespace FolderPreviewBmps_catind
	{
		...
		{
			//addlog(loglevel, ige::LogType::LOG_INFO,  "GetThisFolderBmpPaths in " + path);
			std::vector<std::string> fileNames;
			get_all_filenames_with_extension(path, std::string(), fileNames, false);

			//for(auto& s : fileNames) addlog(loglevel, ige::LogType::LOG_INFO,  "In " + path + ", found " +  s);

			if (std::find(fileNames.begin(), fileNames.end(), previewPng) != fileNames.end())
			{
				std::string pathBmp = path + "\\" + previewPng;
				//addlog(loglevel, ige::LogType::LOG_INFO,  "Registering " + previewPng + " in " + pathBmp);
